...
func TestUpdateLeaks(t *testing.T) {
	...
	for i := byte(0); i < 255; i++ {
		...
		if i%2 == 0 {
			state.SetState(
				addr,
				common.BytesToHash([]byte{i, i, i}),
				common.BytesToHash([]byte{i, i, i, i}),
			)
		}
		if i%3 == 0 {
...
func TestIntermediateLeaks(t *testing.T) {
	...
	transDb, _ := database.NewMemDatabase()
	finalDb, _ := database.NewMemDatabase()
	...
	actions := []testAction{
		...
		},
		{
			...
			fn: func(a testAction, s *StateDB) {
				...
				s.AddLog(&log.Log{Address: addr, Data: data})
			},
			args: make([]int64, 1),
		},
	}
