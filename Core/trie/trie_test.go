...
// Used for testing
func newEmpty() *Trie {
	diskdb, _ := database.NewMemDatabase()
...
func TestMissingRoot(t *testing.T) {
	diskdb, _ := database.NewMemDatabase()
	trie, err := New(
		common.HexToHash("0beec7b5ea3f0fdbc95d0dd47f3c5bc275da8a33"),
		NewDatabase(diskdb),
	)
	if trie != nil {
...
func benchGet(b *testing.B, commit bool) {
	...
	if commit {
		bdb := trie.db.diskdb.(*database.BadgerDB)
		bdb.Close()
		os.RemoveAll(bdb.Path)
	}
}
...
func tempDB() (string, *Database) {
	dir := os.TempDir()
	diskdb, err := database.NewBadgerDB(dir)
	if err != nil {
